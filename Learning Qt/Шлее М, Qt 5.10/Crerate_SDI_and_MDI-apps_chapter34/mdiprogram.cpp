...
DocWindow *MDIProgram::createNewDoc()
{
    ...
    pdoc->setWindowIcon(QPixmap(":/MagentaGuy"));
...
MDIProgram::MDIProgram(QWidget *pwgt):QMainWindow(pwgt)
{
    ...
    pactNew->setIcon(QPixmap(":/RedGuy"));
    ...
    pactOpen->setToolTip("Open an existing file");
    pactOpen->setWhatsThis("Open an existing file");
    pactOpen->setIcon(QPixmap(":/image1"));
    ...
    pactSave->setToolTip("Save the file to disk");
    pactSave->setWhatsThis("Save the file to disk");
    pactSave->setIcon(QPixmap(":/BlackGuy"));
...
{
    DocWindow* pdoc = qobject_cast<DocWindow*>(m_pma->activeSubWindow()->widget());
    if (pdoc) {
        pdoc->slotSave();
    }
}
