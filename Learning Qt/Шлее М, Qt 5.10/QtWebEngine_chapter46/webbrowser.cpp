...
{
    m_ptxt        = new QLineEdit("http://bhv.ru");
...
void WebBrowser::slotFinished(bool bOk)
{
    if (!bOk)
    {
        m_pwv->setHtml("<CENTER> An error has occured while loading the wev page</CENTER>");

    }
    m_ptxt->setText(m_pwv->url().toString());
    m_pcmdBack->setEnabled(m_pwv->page()->history()->canGoBack());
    m_pcmdBack->setEnabled(m_pwv->page()->history()->canGoForward());
